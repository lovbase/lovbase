  ...
  return (
    <div className="h-full flex min-w-0 bg-ink text-fg">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="h-9 shrink-0 flex items-center gap-3 px-3 border-b border-edge font-mono text-[12px]">
          <span className="text-fg">{active}</span>
          {readOnly ? <span className="text-fg-dim">只读 · 由 IR 生成</span> : dirty ? <span className="text-accent-soft">未保存</span> : <span className="text-fg-dim">已保存</span>}
          <span className="ml-auto text-fg-dim">{err}</span>
          {!readOnly && (
            <button onClick={save} disabled={!dirty || saving}
              className="px-2.5 py-1 rounded-md bg-accent text-on-accent text-[11.5px] hover:bg-accent-soft disabled:opacity-40 transition-colors cursor-pointer">
              {saving ? '保存中…' : '保存 ⌘S'}
            </button>
          )}
        </div>
